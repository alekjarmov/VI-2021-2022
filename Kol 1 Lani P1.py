...
class GhostOnSkates(Problem):
    ...
    def successor(self, state):
        ...
        x = state[0]
        y = state[1]
        gore = ["Gore 1", "Gore 2", "Gore 3"]
        desno = ["Desno 1", "Desno 2", "Desno 3"]
        for i, action in enumerate(gore):
            newaction = (x, y + i + 1)
            if self.check_valid(newaction, self.walls, self.n):
                successors[action] = newaction
        for i, action in enumerate(desno):
            newaction = (x + i + 1, y)
            if self.check_valid(newaction, self.walls, self.n):
                successors[action] = newaction

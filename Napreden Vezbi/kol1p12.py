...
class Board(Problem):
    ...
    def __init__(self, initial, n, goal=None):
        ...
        self.changed = [(0, 0), (-1, 0), (1, 0), (0, -1), (0, 1)]
        self.n = n
    ...
    def successor(self, state):
        ...
        for x in range(self.n):
            for y in range(self.n):
                new_coordinates = [(x + dx, y + dy) for dx, dy in self.changed if self.is_valid(x + dx, y + dy)]
                new_state = list(state)
                for i in range(self.n):
                    for j in range(self.n):
                        if (i, j) in new_coordinates:
                            new_state[self.mapper(i, j)] = 1 - state[self.mapper(i, j)]

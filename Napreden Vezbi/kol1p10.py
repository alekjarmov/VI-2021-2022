...
class Squares(Problem):
    def __init__(self, initial, house):
        ...
        self.changes = {'gore': (0, 1), 'dolu': (0, -1), 'levo': (-1, 0), 'desno': (1, 0), }
    ...
    def successor(self, state):
        ...
        for i, el in enumerate(state):
            ...
            for move in self.changes.keys():
                new_state[i] = state[i][0] + self.changes[move][0], state[i][1] + self.changes[move][1]
                if self.check_valid(new_state):
                    succ[f'Pomesti kvadratche {i + 1} {move}'] = tuple(new_state)
        return succ
    ...
    def h(self, node):
        return sum(self.menh(self.goal[i], node.state[i]) for i in range(len(self.goal)))
        # return [self.goal[i] == node.state[i] for i in range(len(self.goal))].count(False)

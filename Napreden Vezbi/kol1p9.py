...
class Table(Problem):
    def __init__(self, initial, walls, m, n, goal=None):
    ...
    def successor(self, state):
        ...
        for ball in state:
            for action in self.moves:
                ...
                i = state.index(ball)
                skipped_ball = ball[0] + self.moves[action][0], ball[1] + self.moves[action][1]
                new_ball = skipped_ball[0] + self.moves[action][0], skipped_ball[1] + self.moves[action][1]
                if skipped_ball in new_state and self.valid_coordinate(new_ball[0], new_ball[1]):
                    new_state[i] = new_ball
                    # new_state.remove(ball)
                    # new_state.append(new_ball)
                    new_state.remove(skipped_ball)
                    new_state = tuple(new_state)
                    successors[f'{action}: (x={ball[0]},y={ball[1]})'] = new_state
        return successors

    def valid_coordinate(self, x, y):
        return (x, y) not in self.walls and min(x, y) >= 0 and x < self.m and y < self.n
    ...
    def goal_test(self, state):
        return len(state) == 1 and state[0][0] == self.m //2 and state[0][1] == self.n -1
...
if __name__ == "__main__":
    ...
    walls = tuple(walls)
    initial = balls
    problem = Table(initial, walls, m, m)

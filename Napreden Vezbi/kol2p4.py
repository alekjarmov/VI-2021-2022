...
def remove_column(data, ind):
    return [row[:ind] + row[ind + 1:] for row in data]


if __name__ == '__main__':
    c = 0.85
    n = len(dataset)
    remove_col = int(input())
    dataset = remove_column(dataset, remove_col)
    xset = [x[:-1] for x in dataset]
    yset = [x[-1] for x in dataset]
    train_x, test_x = xset[:int(n * c)], xset[int(n * c):]
    train_y, test_y = yset[:int(n * c)], yset[int(n * c):]
    num_estimators = int(input())
    criterion = input()
    classifier = RandomForestClassifier(num_estimators, criterion=criterion, random_state=0)
    sample = [list(map(float, input().split()))]
    sample = remove_column(sample, remove_col)

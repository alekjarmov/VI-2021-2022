...
class Pacman(Problem):
    moves = ['ProdolzhiPravo', 'ProdolzhiNazad', 'SvrtiLevo', 'SvrtiDesno']
    walls = \
        [(0, 6), (0, 8), (0, 9),
         (1, 2), (1, 3), (1, 4), (1, 9),
         (2, 9),
    ...
    def __init__(self, initial: tuple, goal=None):
        super().__init__(initial, goal)

        self.coordinates = initial[0]
        self.direction = initial[1]
        self.pills = initial[-1]

    def goal_test(self, state):
        return len(state[-1]) == 0

    def generate_next(self, state, move: str):
        coordinates, direction, pills = state
        directions = ['istok', 'jug', 'zapad', 'sever']
        i_direction = directions.index(direction)
        if move.__contains__('Pravo'):
            new_direction = direction
        elif move.__contains__('Nazad'):
            new_direction = directions[(i_direction + 2) % 4]
        elif move.__contains__('Desno'):
            new_direction = directions[(i_direction + 1) % 4]
        else:
            new_direction = directions[(i_direction - 1) % 4]
        coordinate_map = {'sever': (lambda x, y: (x, y + 1)),
                          'jug': lambda x, y: (x, y - 1),
                          'istok': lambda x, y: (x + 1, y - 0),
                          'zapad': lambda x, y: (x - 1, y - 0), }
        new_coordinates = coordinate_map[new_direction](*coordinates)

        new_pills = tuple(pill for pill in pills if pill != new_coordinates)

        return new_coordinates, new_direction, new_pills
    ...
    def is_valid(self, state):
        ...
        if coordinates in self.walls:
            return False
        return 0 <= min(coordinates) and 10 > max(coordinates)
...
if __name__ == "__main__":
    # problem = Pacman(('f', 'f', 'f'))
    x = int(input())
    y = int(input())
    direction = input()
    pills = tuple(tuple(map(int, input().split(','))) for i in range(int(input())))
    initial = ((x, y), direction, pills)
    print(breadth_first_graph_search(Pacman(initial)).solution())

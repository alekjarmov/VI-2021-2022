...
class Hanoi(Problem):
    ...
    def successor(self, state):
        ...
        for i, pillar1 in enumerate(state):
            for j, pillar2 in enumerate(state):
                if i != j and len(pillar1) != 0:
                    new_state = self.next(state, i, j)
                    # print(new_state)
    ...
    def is_valid(self, state):
        for i, pillar in enumerate(state):
            if list(reversed(sorted(pillar))) != list(pillar):
                # print(f'{pillar} != {reversed(sorted(pillar))}')
                return False
        return True

    def next(self, state, i, j):
        new_state = [list(st) for st in state]
        disk = new_state[i].pop()
        new_state[j].append(disk)
        new_state = tuple(tuple(st) for st in new_state)
        return new_state
...
if __name__ == "__main__":
    # x = tuple(tuple(map(int, i.split(','))) for i in input().split(';'))
    # print(x)
    initial = input()
    goal = input()
    initial = list(initial.split(';'))
    goal = list(goal.split(';'))
    for i, el in enumerate(initial):
        if el == '':
            initial[i] = tuple()
        else:
            initial[i] = tuple(map(int, el.split(',')))
    for i, el in enumerate(goal):
        if el == '':
            goal[i] = tuple()
        else:
            goal[i] = tuple(map(int, el.split(',')))

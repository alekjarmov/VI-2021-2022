...
if __name__ == '__main__':
    ...
    for i, row in enumerate(dataset):
        dataset[i] = [float(el) for el in row]

    train_set = dataset[:round(0.85 * len(dataset))]
    train_x = [row[:-1] for row in train_set]
    train_y = [row[-1] for row in train_set]

    test_set = dataset[round(0.85 * len(dataset)):]
    ...
    #     predicted = classifier.predict([el])
    #     if predicted == test_y[i]:
    #         num_correct += 1
    # print(num_correct / len(test_set))
    print(classifier.score(test_x,test_y))
    lista = list(map(float, input().split(" ")))
    print(f"{classifier.predict([lista])[0]:.0f}")

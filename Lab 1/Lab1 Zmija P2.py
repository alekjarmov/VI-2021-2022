...
class Snake(Problem):
    def __init__(self, initial, goal=None):
        ...
        self.n = 10
        self.m = 10
        self.orientations = {"Levo": (-1, 0), "Dolu": (0, -1), "Desno": (1, 0), "Gore": (0, 1)}
    ...
    def successor(self, state):
        successors = dict()
        moves = ["ProdolzhiPravo", "SvrtiDesno", "SvrtiLevo"]
        for move in moves:
            newstate = self.nextmove(state, move)
            if self.isvalid(newstate):
                successors[move] = newstate
        return successors

    def nextmove(self, state, akcija):
        orientations = ["Levo", "Dolu", "Desno", "Gore"]
        currid = orientations.index(state[1])
        if akcija == "SvrtiDesno":
            orientation = orientations[(currid - 1) % 4]
        elif akcija == "SvrtiLevo":
            orientation = orientations[(currid + 1) % 4]
        else:
            orientation = state[1]
        zmija = list(state[0])
        green = list(state[2])
        new_x = state[0][0][0] + self.orientations[orientation][0]
        new_y = state[0][0][1] + self.orientations[orientation][1]
        zmija = [(new_x, new_y)] + zmija
        if (new_x, new_y) in green:
            green.remove((new_x, new_y))
        else:
            zmija.remove(zmija[len(zmija) - 1])
        green = tuple(green)
        zmija = tuple(zmija)
        return (zmija, orientation, green)
    ...
    def euclid(self, state):
        ...
        manh = []
        # for i, j in state[2]:
        #     sol = max(sol, math.sqrt((x - i) ** 2 + (y - j) ** 2))
        #     manh.append(math.sqrt((x - i) ** 2 + (y - j) ** 2))
        return max(math.sqrt((x - i) ** 2 + (y - j) ** 2) for i, j in state[2])
        # return min(math.sqrt((x-i)**2+(y-j)**2) for i, j in state[2])

    def menhettan(self, state):
        x = state[0][0][0]
        y = state[0][0][1]
        if (len(state[2]) == 0):
            return 0
        ...
        tup2 = (0, 0)
        for i, j in state[2]:
            for xi, yj in state[2]:
                if (abs(xi - i) + abs(yj - j) >= maxi):
                    maxi = abs(xi - i) + abs(yj - j)
                    tup1 = (i, j)
                    tup2 = (xi, yj)
        mini = min(abs(tup1[0] - x) + abs(tup1[1] - y), abs(tup2[0] - x) + abs(tup2[1] - y))
        return mini + maxi

    def euclidmin(self, node):
        state = node.state
        x = state[0][0][0]
        y = state[0][0][1]
        if (len(state[2]) == 0):
    ...
    def h(self, node):
        ...
        state = node.state
        return self.menhettan(state)
        # return max(0, self.euclid(state),len(state[2]),self.menhettan(state))


if __name__ == '__main__':
    initial = [((0, 7), (0, 8), (0, 9)), "Dolu"]
    zel = int(input())
    zeleni = [tuple(map(int, input().split(","))) for i in range(zel)]
    zeleni = tuple(zeleni)
    initial.append(zeleni)

...
class Hanoi(Problem):
    ...
    def successor(self, state):
        successors = dict()
        for c in itertools.permutations(range(self.m), 2):
            action = f"MOVE TOP BLOCK FROM PILLAR {c[0] + 1} TO PILLAR {c[1] + 1}"
            if len(state[c[0]]) == 0:
                continue
            if len(state[c[1]]) == 0:
                newstate = [list(u) for u in state]
                moveddisk = newstate[c[0]][len(newstate[c[0]]) - 1]
                newstate[c[0]].remove(moveddisk)
                newstate[c[1]].append(moveddisk)
                newstate = (tuple(u) for u in newstate)
                successors[action] = tuple(newstate)
                continue
            newstate = [list(u) for u in state]
            moveddisk = newstate[c[0]][len(newstate[c[0]]) - 1]
            cmpdisk=newstate[c[1]][len(newstate[c[1]])-1]
            if moveddisk <= cmpdisk:
                newstate[c[0]].remove(moveddisk)
                newstate[c[1]].append(moveddisk)
                newstate = (tuple(u) for u in newstate)
                successors[action] = tuple(newstate)
...
if __name__ == "__main__":
    ...
    for i in inp.split(";"):
        if (len(i) > 0):
            initial.append(tuple((map(int, i.split(",")))))
        else:
            initial.append(tuple())
    ...
    for i in inp.split(";"):
        if (len(i) > 0):
            goal.append(tuple((map(int, i.split(",")))))
        else:
            goal.append(tuple())
    goal = tuple(goal)
    problem = Hanoi(initial, goal)
    res = uniform_cost_search(problem)
    print(f"Number of action {len(res.solution())}")
    print(res.solution())
    # print(initial)
    # print(goal)
